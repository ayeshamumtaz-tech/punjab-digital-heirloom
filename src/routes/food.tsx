...
export const Route = createFileRoute("/food")({
  head: () => ({
    meta: [
      { title: "Cuisine of Punjab — Saag, Lassi & Street Food | VIRSA" },
      {
        name: "description",
        content:
          "Sarson da saag, makki di roti, lassi in clay kulhars, gol gappay, and Lahore's legendary street food — Pakistani Punjabi cuisine as cultural identity.",
      },
      ...
      { property: "og:image", content: saag },
      { property: "twitter:image", content: saag },
    ],
  }),
  component: FoodPage,
});
...
const dishes = [
  ...
  { name: "Nihari", origin: "Old Lahore", body: "Slow-cooked overnight. Beef shank falling apart. Garam masala, ginger julienne, lemon. Eaten at sunrise with naan torn straight from the tandoor.", img: lassi },
  { name: "Gol Gappay", origin: "Mall Road, Lahore", body: "Crisp puris filled with chickpeas, potato, and an explosive rush of tamarind water. Eaten one after the other until you laugh.", img: saag },
  { name: "Halwa Puri", origin: "Sunday Mornings", body: "Suji halwa, channa masala, fried puris. The breakfast that defines a weekend. Best eaten standing up at a roadside dhaba.", img: saag },
  { name: "Pakoray & Chai", origin: "Monsoon Afternoons", body: "Crisp besan-coated vegetables, fried as the rain begins. Karak doodh patti chai. The whole house gathers at the table.", img: lassi },
];
  ...
  return (
    <>
      <section className="relative h-[70vh] flex items-end overflow-hidden bg-ink">
        <img src={saag} alt="Sarson da saag with makki di roti" className="absolute inset-0 w-full h-full object-cover opacity-70 ken-burns" width={1024} height={1024} />
            ...
            {dishes.map((d, i) => (
              <article key={d.name} className="group bg-card rounded-md overflow-hidden shadow-soft hover-lift" style={{ animationDelay: `${i * 80}ms` }}>
                <div className="image-zoom aspect-[4/3]">
                  <img src={d.img} alt={d.name} loading="lazy" width={1024} height={1024} className="h-full w-full object-cover" />
                </div>
                <div className="p-6">
